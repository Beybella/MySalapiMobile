...
export default function RecordPaymentScreen() {
  ...
  const handleSave = async () => {
    ...
    if (paid > remainingAmount) {
      Alert.alert('Error', `Amount exceeds remaining balance of ₱${remainingAmount.toFixed(2)}`);
      return;
    }
    ...
    if (payError) {
      setLoading(false);
      Alert.alert('Error', payError.message);
      return;
    }
    ...
    setLoading(false);
    Alert.alert('Saved', `Payment of ₱${paid.toFixed(2)} recorded.`, [
      ...
      <ScrollView style={styles.content} keyboardShouldPersistTaps="handled">
        <View style={styles.remainingCard}>
          ...
          <Text style={styles.remainingAmount}>
            ₱{remainingAmount.toLocaleString('en-PH', { minimumFractionDigits: 2 })}
          </Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  content: { flex: 1 },
  remainingCard: {
    margin: 16, backgroundColor: Colors.pautangLedger + '15',
    borderRadius: 16, padding: 20, alignItems: 'center',
    borderWidth: 1, borderColor: Colors.pautangLedger + '30',
  },
  ...
  saveBtn: {
    backgroundColor: Colors.pautangLedger, borderRadius: 12, padding: 16,
