...
export default function ReportsScreen() {
  ...
  const getMonthRange = () => {
    ...
    return {
      start: startOfMonth(base).toISOString().split('T')[0],
      end: endOfMonth(base).toISOString().split('T')[0],
  ...
  const loadData = async () => {
    ...
    const { data: loansGiven } = await supabase
      .from('loans')
      .select('amount, amount_remaining')
                ...
                <View key={cat} style={styles.catRow}>
                  ...
                  <View style={styles.catBarBg}>
                    <View style={[styles.catBarFill, {
                      width: `${pct}%` as any,
                      backgroundColor: CATEGORY_COLORS[cat] || Colors.textLight,
                    }]} />
                  </View>
                </View>
              );
          ...
          <View style={styles.loanSummaryGrid}>
            {[
              ...
              <View key={item.label} style={[styles.loanCard, { borderTopColor: item.color }]}>
                ...
                <Text style={[styles.loanCardValue, { color: item.color }]}>
              ...
              <View key={exp.id} style={styles.txRow}>
                ...
                <View style={styles.txInfo}>
                  ...
                  <Text style={styles.txMeta}>
                    {exp.category} · {format(new Date(exp.expense_date), 'MMM d')}
                  </Text>
                </View>
...
const styles = StyleSheet.create({
  ...
  totalLabel: { fontSize: 13, color: Colors.accentLight, marginBottom: 6 },
  ...
  txRow: {
    backgroundColor: Colors.surface, borderRadius: 10, padding: 14,
    flexDirection: 'row', alignItems: 'center', marginBottom: 6, elevation: 1,
  },
