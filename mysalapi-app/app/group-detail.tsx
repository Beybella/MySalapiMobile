...
export default function GroupDetailScreen() {
  ...
  const loadData = async () => {
    const { data: groupData } = await supabase.from('group_expenses').select('*, payer:payer_id(full_name, email)').eq('id', id).single();
    const { data: participantData } = await supabase.from('group_participants').select('*, participant:participant_id(full_name, email)').eq('group_expense_id', id).order('created_at');
    setGroup(groupData);
  ...
  const markPaid = async (participantId: string) => {
    await supabase.from('group_participants').update({ is_paid: true, paid_at: new Date().toISOString() }).eq('id', participantId);
    const updated = participants.map((p) => p.id === participantId ? { ...p, is_paid: true } : p);
    const allPaid = updated.every((p) => p.is_paid);
    if (allPaid) await supabase.from('group_expenses').update({ status: 'settled' }).eq('id', id);
    loadData();
  };
  ...
  const handleSingil = async (participant: any) => {
    Alert.alert('Send Singil', `Send collection email to ${participant.participant?.email}?`, [
      ...
      { text: 'Send', onPress: async () => {
        const { data: payerProfile } = await supabase.from('users').select('full_name, email').eq('id', user!.id).single();
        const { data: notif } = await supabase.from('email_notifications').insert({ recipient_email: participant.participant?.email, subject_email: `Group Expense Reminder: ₱${participant.share_amount} for ${group?.title}`, notification_type: 'group_singil', subject_cost_id: group?.id, status: 'pending' }).select().single();
        const result = await sendGroupSingil({ recipient_email: participant.participant?.email, payer_name: payerProfile?.full_name || payerProfile?.email || 'Your friend', group_title: group?.title, share_amount: Number(participant.share_amount), notification_id: notif?.id });
        Alert.alert(result.success ? 'Sent!' : 'Failed', result.success ? 'Singil email sent.' : (result.error ?? 'Could not send email.'));
      }},
    ]);
  };
  ...
  const totalCollected = participants.filter((p) => p.is_paid).reduce((s, p) => s + Number(p.share_amount), 0);
    ...
    <View style={styles.container}>
      ...
      <ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={colors.primary} />}>
        <View style={styles.summaryCard}>
          <View style={styles.summaryRow}>
            ...
            <View style={[styles.statusBadge, { backgroundColor: group?.status === 'settled' ? colors.success + '25' : colors.warning + '25' }]}>
              <Text style={[styles.statusText, { color: group?.status === 'settled' ? colors.success : colors.warning }]}>
          ...
          <View style={styles.progressSection}>
            ...
            <View style={styles.progressBg}>
              <View style={[styles.progressFill, { width: participants.length > 0 ? `${(paidCount / participants.length) * 100}%` as any : '0%' }]} />
            </View>
          </View>
        </View>
        ...
        <View style={styles.section}>
          ...
          {participants.length === 0 ? <Text style={styles.emptyText}>No participants added.</Text> : (
            participants.map((p) => (
              <View key={p.id} style={[styles.participantCard, p.is_paid && styles.participantPaid]}>
                <View style={styles.participantLeft}>
                  <View style={[styles.avatar, { backgroundColor: p.is_paid ? colors.success + '25' : colors.ambaganLedger + '25' }]}>
                  ...
                  <View style={{ marginLeft: 12 }}>
                    <Text style={styles.participantName}>{p.participant?.full_name || p.participant?.email}</Text>
                    <Text style={styles.participantEmail}>{p.participant?.email}</Text>
                    {p.is_paid && p.paid_at && <Text style={styles.paidAt}>Paid {format(new Date(p.paid_at), 'MMM d, yyyy')}</Text>}
                  </View>
                </View>
                <View style={styles.participantRight}>
                  <Text style={[styles.shareAmount, p.is_paid && { color: colors.success }]}>{formatCurrency(p.share_amount)}</Text>
                  {p.is_paid ? (
                    <View style={styles.paidBadge}><Ionicons name="checkmark-circle" size={14} color={colors.success} /><Text style={styles.paidText}>Paid</Text></View>
                  ) : isPayer ? (
                    <View style={styles.actionButtons}>
                      <TouchableOpacity style={styles.markPaidBtn} onPress={() => markPaid(p.id)}><Text style={styles.markPaidText}>Mark Paid</Text></TouchableOpacity>
                      <TouchableOpacity style={styles.singilBtn} onPress={() => handleSingil(p)}>
...
const makeStyles = (colors: ReturnType<typeof import('../context/ThemeContext').useTheme>['colors']) =>
  StyleSheet.create({
    ...
    header: { backgroundColor: colors.ambaganLedger, paddingTop: 56, paddingBottom: 16, paddingHorizontal: 16, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
    backBtn: { padding: 8 },
    ...
    participantCard: { backgroundColor: colors.surface, borderRadius: 12, padding: 14, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, elevation: 1 },
